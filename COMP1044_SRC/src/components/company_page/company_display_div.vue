<script setup>
    ...
    // retrieve all data from company
    const retrieve_all = async () => {
        try {
            response.value = await axios.post("../backend/retrieve_company_api.php");
        } catch (error){
            alert(error);
        }
    };
    ...
    // the function to call whenever user hover over an company
    const currently_hover = (company) => {
        current_hover_user.value = company;
    }
...
<template>
    <div v-if="is_login">
        <!-- title -->
        <div class="flex justify-center items-center py-2 font-bold text-2xl border-pink-700 border-b-4 bg-pink-500"><span class="py-1 px-6 bg-pink-300 rounded-full">Company</span></div>
        <div class="flex min-w-max max-w-screen min-h-min h-screen max-h-screen bg-gradient-to-r bg-linear-to-bl from-violet-500 to-fuchsia-500 overflow-auto justify-center">
                ...
                <div class="flex flex-col h-full overflow-auto last:border-r-0">
                    ...
                    <div class="flex flex-col w-max overflow-y-auto h-full bg-rose-100" v-if="need_display && JSON.stringify(response.data) != '[]'">
                        <div v-for="response_value in response.data" :key="response_value">
                            <router-link :to="{name: 'company_edit_page', params: {company_id: response_value['company_id']}}" @mouseover="currently_hover(response_value)" class="w-max min-w-180 min-h-10 bg-rose-100 hover:bg-rose-300 flex flex-row border-b-pink-700/80 border-b-1 overflow-hidden" v-if="response.status==200" v-bind:id="response_value.individual_id">
                    ...
                    <div class="flex bg-red-400 h-full w-full justify-center items-center" v-if="JSON.stringify(response.data) == '[]'">
                        <span>No company fulfill the search requirement</span>
                    </div>
                </div>
            ...
            <div class="flex flex-col min-h-max max-h-max w-180 max-w-180 min-w-180 m-2">
                <div class="flex flex-col px-2 py-1 bg-pink-300 border-pink-700 rounded-lg border-3 m-1">
                    <div class="h-max font-bold mx-1 text-xl py-1 decoration-2 decoration-pink-700 underline">Filter Component</div>
                    ...
                    <div class="flex flex-row overflow-auto bg-rose-100 border-pink-700 rounded-md border-3">
                        <div class="w-1/2 h-full flex flex-col">
                            <div class="w-full flex flex-row px-2 m-1">
                                <div class="w-30 border-pink-700 border-b-2 font-semibold">{{ preview_attribute_name['company_name']['name'] }}</div>
                                <div class="mr-2 font-semibold border-pink-700 border-b-2">:</div>
                                <div class="w-52 text-nowrap overflow-auto border-pink-700 border-b-2">{{ current_hover_user['company_name'] }}</div>
                            </div>
                            ...
                            <div class="flex flex-col px-2 m-1">
                                <div class="font-semibold border-pink-700 border-b-2 w-full">{{ preview_attribute_description['company_description']['name'] }}:</div>
                        ...
                        <div class="w-1/2 h-full flex flex-col">
                            ...
                            <div class="flex flex-col px-2 m-1">
                                <div class="font-semibold border-pink-700 border-b-2">{{ preview_attribute_description['company_address']['name'] }}:</div>
                                <div class="flex overflow-auto h-40 p-2">{{ current_hover_user[preview_attribute_description['company_address']['correspond']] }}</div>
                            </div>
                        </div>
                    </div>
                    <div class="flex flex-row justify-end bg-pink-500 border-pink-700 border-3 mt-2 rounded-md">
                        <router-link :to="{name: 'company_edit_page', params: {company_id: current_hover_user['company_id']}}" tag="button"><div class="w-35 bg-pink-200 mx-3 hover:bg-pink-700 hover:text-white rounded-full flex justify-center items-center m-2 font-semibold py-1">Open in detail</div></router-link>
                    </div>
                </div>
                
                <!-- create new individual -->
                <div class="bg-pink-300 rounded-md border-pink-700 border-3 m-1 h-20 flex justify-end">
                    <router-link class="mx-3 my-4 bg-green-500 py-2 px-4 rounded-full font-bold hover:bg-green-800 hover:text-white" :to="{name: 'company_creation_page'}" tag="button">Create new company</router-link>
                </div>
