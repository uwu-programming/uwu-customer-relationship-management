...
function retrieve_customer($conn){
    try {
        ...
        $sql_query = "SELECT * FROM individual LEFT JOIN country ON individual.country_code = country.country_code LEFT JOIN company ON individual.company_id = company.company_id LEFT JOIN (SELECT user_id AS created_by, user_name AS created_by_name FROM crm_user) t1 USING(created_by) WHERE relationship = 'Customer'";
